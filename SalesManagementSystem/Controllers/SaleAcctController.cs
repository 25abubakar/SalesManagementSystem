...
namespace SalesManagementSystem.Controllers
{
    public class SaleAcctController : Controller
    {
        ...
        private async Task AssignCompanyFromPlatformAsync(SaleAcctCreateVM model)
        {
            ...
            if (model.CompanyId.HasValue && model.CompanyId.Value != platform.CompanyId.Value)
            {
                ModelState.AddModelError(nameof(model.CompanyId), "Selected company does not match selected platform.");
                return;
            }
